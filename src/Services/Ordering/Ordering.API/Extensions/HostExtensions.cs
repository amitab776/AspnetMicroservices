...
namespace Ordering.API.Extensions
{
    public static class HostExtensions
    {
        ...
        {
            int retyrForAvailability = retry.Value;
            ...
            using(var scope = host.Services.CreateScope())
            {
                ...
                {
                    logger.LogError(ex, "Migration Database associated with context {dbContextName}", nameof(TContext));

                    if(retyrForAvailability < 50)
                    {
                        retyrForAvailability++;
                        System.Threading.Thread.Sleep(2000);
                        MigrateDatabase<TContext>(host, seeder, retyrForAvailability);
                    }
                    throw;
                }
            }
